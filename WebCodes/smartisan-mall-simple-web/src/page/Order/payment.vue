  ...
  <div class="w" style="padding-bottom: 100px;">
        ...
        <div>
          <div class="box-inner">
            <div>
              ...
              <y-button text="立刻支付" classStyle="main-btn" style="width: 120px;height: 40px;font-size: 16px;line-height: 38px" @btnClick="paySuc()"></y-button>
            </div>
          </div>
        </div>
    ...
    <div class="p-msg w">
      <div class="confirm-detail">
        ...
        <p class="info-detail">详细地址：{{addList.streetName}}</p>
      </div>
    </div>
      ...
      <div class="cart-items" v-for="(item,i) in cartList" :key="i" v-if="item.checked === '1'">
        <div class="name">
          <div class="name-cell ellipsis">
            <a href="javascript:;" title="" target="_blank">{{item.productName}}</a></div>
        </div>
        <div class="n-b">
          <div class="subtotal ">
...
import { addressListById, getCartList, payMent, productDet } from '/api/goods'
export default {
  data () {
    return {
      payType: 1,
  ...
  methods: {
    ...
    _addressList (params) {
      addressListById(params).then(res => {
        this.addList = res.result[0]
      })
    },
    paySuc () {
      payMent({
        addressId: this.addressId.toString(),
        orderTotal: this.checkPrice.toString(),
        productId: this.productId,
        productNum: this.num.toString()
      }).then(res => {
        if (res.status === '0') {
  ...
  created () {
    ...
    if (this.addressId) {
      this._addressList({ addressId: this.addressId.toString() })
